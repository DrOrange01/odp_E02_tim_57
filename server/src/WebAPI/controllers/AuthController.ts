...
import { validacijaPodatakaAuth } from "../validators/auth/RegisterValidator.ts";
  ...
  private async prijava(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (result.id !== 0) {
        ...
        res
          .status(200)
          .json({ success: true, message: "Uspešna prijava", data: token });
        return;
      } else {
        res.status(401).json({
          success: false,
          message: "Неисправно корисничко име или лозинка",
        });
        return;
      }
    } catch (error) {
