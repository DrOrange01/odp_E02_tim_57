import type { RezultatValidacije } from "../../../types/validations/ValidationResult";
export function validacijaPodatakaAuth(
  ...
  if (korisnickoIme.length < 3) {
    return {
      uspesno: false,
      poruka: "Korisničko ime mora imati najmanje 3 karaktera.",
    };
  }
  if (lozinka.length < 6) {
    return {
  ...
  if (lozinka.length > 20) {
    return {
      uspesno: false,
      poruka: "Lozinka može imati najviše 20 karaktera.",
    };
  }
  return { uspesno: true };
}
